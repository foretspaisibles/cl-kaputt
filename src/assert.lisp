...
(define-condition assertion-failed (serious-condition)
  ((assertion-name :accessor assertion-name :initarg :assertion-name)
      ...
      `(restart-case
	   (if (progn ,@body)
	       ...
	       (error 'assertion-failed
		      :assertion-name ,(symbol-name assertion-name)
		      :assertion-description ,description
		      :assertion-form *assertion-form*
		      :assertion-args *assertion-args*))
	 (continue ()
	   :report
	   (lambda (stream)
...
context to this informational messsage, like the value of some variables."
  (labels
	 ...
	 (let ((defun-body
		 (define-assertion/defun-body assertion-name docstring report body)))
...
boolean validating details of the assertion."
  (let ((condition-validation
	  (cond
	    ((and slots body)
	     (list
	      (define-assertion/defun-body 'assert-condition nil nil 
    ...
    `(handler-case
	 (let ((form-value ,form))
	   (error 'assertion-failed
		  :assertion-name "ASSERTION-CONDITION"
		  :assertion-description
		  (format nil "The form ~S was expected to signal a condition ~S~
	 ...
	 (error 'assertion-failed
		...
		:assertion-description
		(format nil "The form ~S was expected to signal a condition ~S~
                             but it yielded a condition ~S instead.~%~%  ~S"
